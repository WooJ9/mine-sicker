...
CAPTION = "minesweeper"
ICON = pg.image.load("image/mine.png")
...
mine_image = pg.transform.scale(pg.image.load("image/mine.png"), [cell_size, cell_size])
flag_image = pg.transform.scale(pg.image.load("image/flag.png"), [cell_size, cell_size])
not_mine_image = pg.transform.scale(pg.image.load("image/not_mine.png"), [cell_size, cell_size])
tab_size = 36
vic = False
dft = False
...
def print_board():
    for i in game_board.content:
        for j in i:
            print(1 if j.has_mine else 0, end = ' ')
        print()
